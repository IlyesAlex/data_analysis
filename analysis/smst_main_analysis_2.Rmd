...
smst_stim_list_complex %>% 
  ...
  filter(covariate %in% c("ya_ert_mean", "ya_konk_mean", "ya_aro_mean", "oa_ert_mean", "oa_konk_mean", "oa_aro_mean")) %>% 
  ggplot(aes(x = factor(stimtype_REC, levels= c("TARG", "CLOSE", "DISTANT", "FOIL"), labels=c("Targets","Close\nLures", "Distant\nLures", "Foils")), y = as.numeric(result))) +
    ...
    facet_wrap(~covariate, scales = "free", labeller = labeller(covariate = c(#"noun_freq"="Noun frequency",
                                                                              ...
                                                                              "oa_konk_mean" = "Concreteness (34 O)",
                                                                              "oa_aro_mean" = "Arousal (34 O)"))) +
    custom_theme +
...
model_complex_participants_cognitive_health = glmer(response_REC ~ stimtype_REC * AgeGroup +
                                        ...
                                        stimtype_REC * AgeGroup * MST_ldi_scale +
                                        (1|trialnum_REC) + 
                                        (1 + exercise_regular * sedentary_regular * sleep_regular|ID) + 
                                        (1|itemno),
                                        family = binomial("probit"), 
                                        data = semmst_data_final_clean_GLMEM,
...
```{r}
summary(model_optimal_participants_cognitive_health)
...
# Baseline model - categorical
model_cat_base <- glmer(correctness ~ stimtype_REC * AgeGroup + (1 | trialnum_REC), family = binomial("logit"), data = semmst_data_final_clean)

summary(model_cat_base)

# Model with more random effects - categorical
model_cat_A <- glmer(correctness ~ stimtype_REC * AgeGroup + (1 | trialnum_REC) + (1|ID) + (1|itemno), family = binomial("logit"), data = semmst_data_final_clean)

summary(model_cat_A)

# Model with more random effects - categorical + 2 random effects
model_cat_B <- glmer(correctness ~ stimtype_REC * AgeGroup + (1 | trialnum_REC) + (1 + scale(Digit_symbol)|ID) + (1 + scale(ya_ert_mean)|itemno), family = binomial("logit"), data = semmst_data_final_clean)

summary(model_cat_B)

# Model with more random effects - categorical + 4 random effects
model_cat_C <- glmer(correctness ~ stimtype_REC * AgeGroup + (1 | trialnum_REC) + (1 + scale(Digit_symbol) + scale(Vocabulary)|ID) + (1 + scale(ya_ert_mean) + scale(noun_freq)|itemno), family = binomial("logit"), data = semmst_data_final_clean)

summary(model_cat_C)


# Model with more random effects - categorical + 4 random effects
model_cat_D <- glmer(correctness ~ stimtype_REC * AgeGroup + (1 | trialnum_REC) + (1 + scale(Digit_symbol) + scale(Vocabulary) + scale(MST_ldi)|ID) + (1 + scale(ya_ert_mean) + scale(noun_freq) + scale(adj_freq)|itemno), family = binomial("logit"), data = semmst_data_final_clean)

summary(model_cat_D)


# Model with more random effects - categorical + 4 random effects
model_cat_E <- glmer(correctness ~ stimtype_REC * AgeGroup + (1 | trialnum_REC) + (1 + scale(Digit_symbol) + scale(Vocabulary) + scale(MST_ldi) + scale(exercise_regular)|ID) + (1 + scale(ya_ert_mean) + scale(noun_freq) + scale(adj_freq) + scale(ya_aro_mean)|itemno), family = binomial("logit"), data = semmst_data_final_clean)

summary(model_cat_E)

# Model with more random effects - categorical + 4 random effects
model_cat_Y <- glmer(correctness ~ stimtype_REC * AgeGroup + (1 | trialnum_REC) + (1 + scale(Digit_symbol) + scale(Vocabulary) + scale(MST_ldi) + scale(exercise_regular) + scale(Beck)|ID) + (1 + scale(ya_konk_mean) + scale(noun_freq) + scale(adj_freq) + scale(ya_aro_mean)|itemno), family = binomial("logit"), data = semmst_data_final_clean)

summary(model_cat_Y)


anova(model_cat_A, model_cat_E)


# Model with ID covariate random effects - categorical
model_cat_id <- glmer(correctness ~ stimtype_REC * AgeGroup + (1 | trialnum_REC) + (1 + Beck + Vocabulary + Digit_symbol + Non_word_sum + MST_ldi + MST_rec_score + exercise_regular + sedentary_regular + sleep_regular|ID) + (1|itemno), family = binomial("logit"), data = semmst_data_final_clean, control = glmerControl(optimizer ='optimx', optCtrl=list(method='nlminb', maxfun = 1e6)))

summary(model_cat_id)

all_fit_B = allFit(model_cat_B, maxfun = 1e6)

# Model with item covariate random effects - categorical
model_cat_cov <- glmer(correctness ~ stimtype_REC * AgeGroup + (1 | trialnum_REC) + (1|ID) + (1 + noun_freq + adj_freq + adj_syll + noun_syll + ya_ert_mean + ya_aro_mean |itemno), family = binomial("logit"), data = semmst_data_final_clean, control = glmerControl(optimizer ='optimx', optCtrl=list(method='nlminb')))

summary(model_cat_cov)

# Model most complex - categorical
model_cat_X <- glmer(correctness ~ stimtype_REC * AgeGroup + (1 | trialnum_REC) + (1 + Beck + Vocabulary + Digit_symbol + Non_word_sum + MST_ldi + MST_rec_score + exercise_regular + sedentary_regular + sleep_regular|ID) + (1 + noun_freq + adj_freq + adj_syll + noun_syll + ya_ert_mean + ya_aro_mean |itemno), family = binomial("logit"), data = semmst_data_final_clean, control = glmerControl(optCtrl = list(maxfun = 200000)), nAGQ=0)

summary(model_cat_X)



# Baseline model - continous
model_cont_base <- glmer(correctness ~ cosine_metric * AgeGroup + (1 | trialnum_REC), family = binomial("logit"), data = semmst_data_final_clean)

summary(model_cont_base)

# Model with more random effects - categorical
model_cont_A <- glmer(correctness ~ cosine_metric * AgeGroup + (1 | trialnum_REC) + (1|ID) + (1|itemno), family = binomial("logit"), data = semmst_data_final_clean)

summary(model_cont_A)



plot(ggpredict(base_model_cont, terms = "cosine_metric"))
...
cor.test(semmst_data_final$MST_ldi, semmst_data_final$close_dprime)
cor.test(semmst_data_final$MST_ldi, semmst_data_final$distant_dprime)

# MST - smst REC dprime
cor.test(semmst_data_final$MST_rec_score, semmst_data_final$rec_dprime)
...
semmst_data_final %>% 
  ...
  ggplot(mapping = aes(x = MST_rec_score, y = rec_dprime, fill = AgeGroup, colour = AgeGroup)) +
    ...
    custom_theme +
    labs(x = "Recognition score in MST", y = "D-prime scores - discriminability", title = "Correlation of dprimes in our main task\nand the recognition scores in the original MST") +
