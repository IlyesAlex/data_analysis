...
semmst_data_final_clean %>%
  select(adjective_REC, noun, AgeGroup, stimtype_REC, response_REC, cosine_metric) %>%
  group_by(AgeGroup, stimtype_REC, response_REC, noun) %>% 
  mutate(n = n()) %>% 
  ungroup() %>% 
  distinct() %>% 
  complete(nesting(AgeGroup), 
           nesting(stimtype_REC, noun, adjective_REC, response_REC, cosine_metric),
           fill = list(n = 0)) %>%
  group_by(AgeGroup, stimtype_REC, noun) %>%
  mutate(response_rate=n/sum(n)) %>% 
  ungroup() %>% 
  ...
  fill(cosine_metric) %>% print(n = 900) %>% 
  ggplot(aes(x = cosine_metric, y = response_rate)) +
    geom_point(size = 6, colour = "#4472C4", alpha = 0.75) +
    geom_smooth(method= "lm", colour = "#5A5A5A", size = 1.2) +
    custom_theme +
    #ggtitle("False alarms to verbal similar lures based on \n semantic similarity in word2vec by 40 young adults") +
    xlab("Semantic similarity") + ylab("False alarm rate") +
