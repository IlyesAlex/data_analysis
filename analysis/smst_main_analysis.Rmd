...
## SUPPLEMENTARY - item covariate t-tests

```{r}

item_covariate_tests = semmst_data_final_clean %>% 
  select(ID, AgeGroup, stimtype_REC, response_REC, reaction_time_REC, ya_ert_mean, ya_konk_mean, ya_aro_mean, oa_ert_mean, oa_konk_mean, oa_aro_mean) 
...
# Boxplot
dprimes_table %>% 
  ggplot(aes(x = factor(type, labels = c("TARGET vs.\nCLOSE LURE", "TARGET vs.\nDISTANT LURE", "TARGET vs.\nFOIL")), y = dprime)) +
    geom_boxplot(aes(colour = factor(type, labels = c("TARGET vs.\nCLOSE LURE", "TARGET vs.\nDISTANT LURE", "TARGET vs.\nFOIL"))), lwd=1.3) +
    geom_point(aes(x = factor(type, labels = c("TARGET vs.\nCLOSE LURE", "TARGET vs.\nDISTANT LURE", "TARGET vs.\nFOIL")), y = dprime), position = position_jitter(width=0.1), size = 3) +
    #scale_colour_brewer(palette="Dark2") +
    scale_colour_manual(values = c("#306EFF", "#FFEF00",  "#E41A1C")) +
    labs(x = "Type of d' scores (contrasts)", y = "D' score - discriminability", title = "D' scores for different contrasts split by age group", colour = "Type of d' scores (contrasts)") +
    custom_theme +
    facet_wrap(~AgeGroup, labeller = labeller(AgeGroup=c("OA" = "Old Adults", "YA" = "Young Adults"))) +
    theme(legend.position = "none")
...
semmst_data_final_clean %>%
  ...
  ggplot(mapping = aes(x = MST_ldi, y = lure_dprime, fill = AgeGroup, colour = AgeGroup)) +
    ...
    custom_theme +
    labs(x = "MST LDI", y = "sMST - Lure D'", title = "Correlation of dprimes in the sMST\nand the LDI scores in the original MST") +
    theme(legend.position = "none") + guides(colour = FALSE)
