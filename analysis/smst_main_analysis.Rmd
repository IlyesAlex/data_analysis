...
# Packages needed and theme for plots
...
custom_theme = theme(
      ...
      axis.title.x = element_text(vjust = -2, hjust=0.5, size = 26, face="bold", margin = margin(b = 30)),
...
## Create D-prime dataframe for ANOVA
...
semmst_data_rec_wide_dprime_lurebinned = semmst_data_rec %>%
  filter(reaction_time <= 3) %>% 
  filter(response != "None" & response != "4") %>% 
    mutate(stimtype = recode(stimtype, 'CLOSE' = 'LURE', 'DISTANT' = 'LURE')) %>%
  count(ID, AgeGroup, stimtype, response) %>%
  unite("stim_resp", c("stimtype", "response"), sep = "_") %>% 
  spread(stim_resp, n) %>% 
  mutate_all(~ifelse(is.na(.), 0, .)) %>% 
...
dprimes_wide_binned_norec = dprimes_table_wide_lurebinned %>% 
  select(-rec_dprime)
...
### FINAL MERGE
semmst_data_final = Reduce(function(x, y) merge(x, y, all=TRUE), list(semmst_data_wider_cosine, mst_sum, nback_sum, palt_sum, heatlh_sum, dprimes_wide_binned_norec, dprimes_table_wide))
...
semmst_data_final_clean = semmst_data_final_clean%>% 
  mutate(across(c(ID, itemno, Experimenter, confidence, stimtype_ENC, response_ENC, response_REC, correctness, alcohol_24h, caffeine_24h, sleep_24h), as.factor)) %>% 
  mutate(alcohol_regular = factor(alcohol_regular, ordered=TRUE)) %>%
  mutate(caffeine_regular = factor(caffeine_regular, ordered=TRUE)) %>%  
  mutate(sleep_regular = factor(sleep_regular, ordered=TRUE)) %>%  
  mutate(exercise_regular = factor(exercise_regular, ordered=TRUE)) %>%  
...
Get mean age and distribution of sex
...
smst_enc_rtdata = semmst_data_final_clean %>% 
  group_by(ID, AgeGroup) %>% 
  summarise(mean_enc_rt = mean(reaction_time_ENC)) %>% 
  ungroup()

smst_rec_rtdata = semmst_data_final_clean %>% 
  group_by(ID, AgeGroup) %>% 
  summarise(mean_rec_rt = mean(reaction_time_REC)) %>% 
  ungroup()
  
t.test(mean_enc_rt ~ AgeGroup, data = smst_enc_rtdata)
## OA mean: 1.243; YA mean: 1.273
## t = -1.1435, df = 78, p = 0.2563
t.test(mean_rec_rt ~ AgeGroup, data = smst_rec_rtdata)
## OA mean: 1.74; YA mean: 1.447
## t = 6.7932, df = 78, p < .001
...
## t = -4.06, df = 76.97, p = 0.00012
## OA mean: 0.73 YA mean: 0.81
...
## t = 3.34, df = 71.36, p-value = 0.001321
## OA mean: 0.82 YA mean: 0.74
...
### Further analyses - SciRep review (2024 june)
...
semmst_data_final_clean_ANOVA_MST = semmst_data_final %>% 
  select(ID, AgeGroup, MST_ldi, lure_dprime) %>% 
  distinct() %>% 
  gather("modality", "discriminability", c("MST_ldi", "lure_dprime")) %>% 
  mutate(modality = ifelse(modality == "MST_ldi", "MST", "sMST")) %>% 
  mutate(modality = as.factor(modality)) %>% 
  mutate(AgeGroup = as.factor(AgeGroup)) %>% 
  filter(modality == "MST")

semmst_data_final_clean_ANOVA_sMST = semmst_data_final %>% 
  select(ID, AgeGroup, MST_ldi, lure_dprime) %>% 
  distinct() %>% 
  gather("modality", "discriminability", c("MST_ldi", "lure_dprime")) %>% 
  mutate(modality = ifelse(modality == "MST_ldi", "MST", "sMST")) %>% 
  mutate(modality = as.factor(modality)) %>% 
  mutate(AgeGroup = as.factor(AgeGroup)) %>% 
  filter(modality == "sMST") %>% 
  mutate(discriminability = normalize(discriminability, method="range"))

semmst_data_final_clean_ANOVA_merge = semmst_data_final %>% 
  select(ID, AgeGroup, MST_ldi, lure_dprime) %>% 
  distinct() %>% 
  gather("modality", "discriminability", c("MST_ldi", "lure_dprime")) %>% 
  mutate(modality = ifelse(modality == "MST_ldi", "MST", "sMST")) %>% 
...
modality_anova <- aov(discriminability ~ AgeGroup * modality, data = semmst_data_final_clean_ANOVA_MST)
summary(modality_anova)
...
## Returns : 2., 29. observations -> IDs: 006150 - MST check?;  421568 - MST check + she knew the MST, but she was confused by the task "which one did you ask for?"
## Returns: 24., 33. observations -> IDs: 392539 - Digit Symbol 95 outlier; 449122 - Vocabulary 8 outlier
...
semmst_data_final_clean_GLMEM = semmst_data_final_clean %>% 
  ...
  mutate(stimtype_REC = relevel(stimtype_REC, "FOIL")) %>% 
...
#### Simple model with only two fixed effects in interaction
...
# Pseudo-r2
r2(model_simple)
augment(model_simple, type.predict = "response")
plot(performance_roc(model_simple))
...
### TARG: estimate = 8.96, p < .001
### CLOSE: estimate = 1.92, p < .001
### DISTANT: estimate = 1.17, p = .096
#### N.s. AgeGroup effect
### Age: estimate = 1.44, p = .42
#### Significant interactions
### TARG * Age: estimate = .53, p < .001
### DISTANT * Age: estimate = .78, p = .05
#### N.s. interaction
### CLOSE * Age: estimate = 1.01, p = .91
#### Covariates
### AgeYA * Digit_Symbol: estimate = 0.83, p = .03
### AgeOA * Non_word_sum_scale: estimate = 1.15, p = .02
### AgeYA * MST_ldi: estimate = 0.88, p = 0.05
### FOIL * noun_freq: estimate = 1.08, p = .02
### TARG * noun_freq: estimate = 0.93, p = .01
### CLOSE * AgeYA * oa_konk: estimate = 0.81, p = .04
### CLOSE * AgeYA * ya_konk: estimate = 1.47, p < .001
### DISTANT * AgeYA * ya_konk:estimate = 1.31, p < .01
### CLOSE * AgeOA * ya_konk: estimate = 1.20, p = .04

anova(model_simple, model_complex_participants_cognitive_health, test = "Chisq")
...
semmst_data_final_clean_GLMEM_cont_dist = semmst_data_final_clean_GLMEM %>% 
  filter(stimtype_REC != "TARG") %>% 
  filter(stimtype_REC != "FOIL") %>% 
  filter(stimtype_REC != "CLOSE")

semmst_data_final_clean_GLMEM_cont_close = semmst_data_final_clean_GLMEM %>% 
  filter(stimtype_REC != "TARG") %>% 
  filter(stimtype_REC != "FOIL") %>% 
  filter(stimtype_REC != "DISTANT") %>% 
  filter(cosine_metric < .89)

semmst_data_final_clean_GLMEM_cont_lures = semmst_data_final_clean_GLMEM %>% 
  filter(stimtype_REC != "FOIL") 

  
  
model_simple_cont_only_distant = glmer(response_REC ~ AgeGroup * cosine_metric +
                                        (1|trialnum_REC) + 
                                        (1|ID) + 
                                        (1|itemno),
                                        family = binomial("probit"), 
                                        data = semmst_data_final_clean_GLMEM_cont_dist,
                                        control = glmerControl(optimizer ='optimx', optCtrl=list(method='nlminb', maxfun = 1e6)))

model_simple_cont_only_close = glmer(response_REC ~ AgeGroup * cosine_metric +
                                        (1|trialnum_REC) + 
                                        (1|ID) + 
                                        (1|itemno),
                                        family = binomial("probit"), 
                                        data = semmst_data_final_clean_GLMEM_cont_close,
                                        control = glmerControl(optimizer ='optimx', optCtrl=list(method='nlminb', maxfun = 1e6)))

model_simple_cont_only_lures = glmer(response_REC ~ AgeGroup * cosine_metric +
                                        (1|trialnum_REC) + 
                                        (1|ID) + 
                                        (1|itemno),
                                        family = binomial("probit"), 
                                        data = semmst_data_final_clean_GLMEM_cont_lures,
                                        control = glmerControl(optimizer ='optimx', optCtrl=list(method='nlminb', maxfun = 1e6)))
...
summary(model_simple_cont_only_lures)
summary(model_simple_cont_only_close)
summary(model_simple_cont_only_distant)

# OR
broom.mixed::tidy(model_simple_cont_only_lures, conf.int = TRUE, exponentiate = TRUE)
broom.mixed::tidy(model_simple_cont_only_close, conf.int = TRUE, exponentiate = TRUE)
broom.mixed::tidy(model_simple_cont_only_distant, conf.int = TRUE, exponentiate = TRUE)

r2(model_simple_cont_only_lures)
r2(model_simple_cont_only_close)
r2(model_simple_cont_only_distant)


# Plotting ratio
semmst_data_final_clean_GLMEM %>% 
  mutate(response_REC = ifelse(response_REC == "old", 1, 0)) %>%
  filter(stimtype_REC != "FOIL") %>% 
  ggplot() +
    ...
    geom_smooth(method = "glm", 
                method.args = list(family = "binomial")) +
    coord_cartesian(ylim = c(0, 1), xlim = c(0.4,1)) +
    scale_y_continuous(labels = scales::percent_format()) +
    custom_theme +
    labs(title = "Continous GLMEM model plotted", y = "Percentage of 'old' responses", x = "Cosine Similarity")
...
### ANOVA with D'a (Loiotile, 2015) - NOT USING
...
semmst_data_final %>% 
  ...
  ggplot(mapping = aes(x = MST_ldi, y = lure_dprime, fill = AgeGroup, colour = AgeGroup)) +
    ...
    sm_statCorr(fit.params = list(color = 'black',
                                   linetype = 'dashed'), text_size = 10) +
    custom_theme +
    labs(x = "Lure Discrimination Index in MST", y = "D-prime scores of targ. vs. all lures - discriminability", title = "Correlation of dprimes in the sMST\nand the LDI scores in the original MST") +
    theme(legend.position = "none") + guides(colour = FALSE)
```
...
## Health and Lifestyle - Descriptives

```{r}

health_data_supplement = read_excel("data/D_PencilPaper/paper_pencil_logs.xlsx", sheet = 2) 

# Question 1
health_data_supplement %>%
  select(ParticipantID, AgeGroup, Alcohol_24hours) %>% 
  group_by(AgeGroup, Alcohol_24hours) %>% 
  summarise(cnt = n()) %>%
  mutate(freq = round(cnt / sum(cnt), 3))
  
# Question 2
health_data_supplement %>%
  select(ParticipantID, AgeGroup, Alcohol_24hours_amount) %>%
  mutate(Alcohol_24hours_amount = as.numeric(substr(Alcohol_24hours_amount, 1, 1))) %>% 
  group_by(AgeGroup) %>% 
  filter(Alcohol_24hours_amount != 0) %>% 
  summarise(mean = mean(Alcohol_24hours_amount, na.rm = TRUE), sd = sd(Alcohol_24hours_amount, na.rm = TRUE))
  
# Question 3
health_data_supplement %>%
  group_by(AgeGroup) %>% 
  summarise(median = median(as.numeric(Alcohol_regular_frequency), na.rm =TRUE))

# Question 4
health_data_supplement %>%
  group_by(AgeGroup) %>%
  select(AgeGroup, Alcohol_regular_amount) %>%
  mutate(Alcohol_regular_amount = as.numeric(substr(Alcohol_regular_amount, 1, 1))) %>% 
  filter(Alcohol_regular_amount != 0) %>% 
  summarise(mean = mean(Alcohol_regular_amount, na.rm = TRUE), sd = sd(Alcohol_regular_amount, na.rm = TRUE))

# Question 5
health_data_supplement %>%
  group_by(AgeGroup, Caffeine_24hours) %>% 
  summarise(cnt = n()) %>%
  mutate(freq = round(cnt / sum(cnt), 3))
...
health_data_supplement %>%
  group_by(AgeGroup) %>% 
  mutate(Caffeine_24hours_amount = as.numeric(substr(Caffeine_24hours_amount, 1, 1))) %>% 
  filter(Caffeine_24hours_amount != 0) %>% 
  summarise(mean = mean(Caffeine_24hours_amount, na.rm = TRUE), sd = sd(Caffeine_24hours_amount, na.rm = TRUE))

# Question 7
health_data_supplement %>%
  group_by(AgeGroup) %>% 
  select(AgeGroup, Caffeine_regular_types) %>%  print(n=80)
  
# Question 8
health_data_supplement %>%
  group_by(AgeGroup) %>% 
  summarise(median = median(as.numeric(Caffeine_regular_frequency), na.rm =TRUE))

# Question 9
health_data_supplement %>%
  group_by(AgeGroup) %>% 
  mutate(Caffeine_regular_amount = as.numeric(substr(Caffeine_regular_amount, 1, 1))) %>% 
  filter(Caffeine_regular_amount != 0) %>% 
  summarise(mean = mean(Caffeine_regular_amount, na.rm = TRUE), sd = sd(Caffeine_regular_amount, na.rm = TRUE))

# Question 10
health_data_supplement %>%
  group_by(AgeGroup, Nicotine_24hours) %>% 
  drop_na() %>% 
  summarise(cnt = n()) %>%
  mutate(freq = round(cnt / sum(cnt), 3))
...
# Question 11
health_data_supplement %>%
  select(AgeGroup, Nicotine_24hours_amount) %>%
  group_by(AgeGroup) %>% 
  drop_na() %>% print(n=80)  
  mutate(Nicotine_24hours_amount = as.numeric(substr(Nicotine_24hours_amount, 1, 1))) %>% 
  filter(Nicotine_24hours_amount != 0) %>% 
  summarise(mean = mean(Nicotine_24hours_amount, na.rm = TRUE), sd = sd(Nicotine_24hours_amount, na.rm = TRUE))

# Question 12
health_data_supplement %>%
  select(AgeGroup, Nicotine_regular_types) %>% 
  group_by(AgeGroup) %>%
  drop_na() %>%  print(n=80)
  
# Question 13
health_data_supplement %>%
  select(AgeGroup, Nicotine_regular_frequency) %>%
  group_by(AgeGroup) %>%
  drop_na() %>%  
  summarise(median = median(as.numeric(Nicotine_regular_frequency), na.rm =TRUE))
  
# Question 14
health_data_supplement %>%
  select(AgeGroup, Nicotine_regular_amount) %>%
  group_by(AgeGroup) %>%
  drop_na() %>% print(n=80)
  mutate(Nicotine_regular_amount = as.numeric(substr(Nicotine_regular_amount, 1, 1))) %>% 
  filter(Nicotine_regular_amount != 0) %>% 
  summarise(mean = mean(Nicotine_regular_amount, na.rm = TRUE), sd = sd(Nicotine_regular_amount, na.rm = TRUE))
  
# Question 15
health_data_supplement %>%
  group_by(AgeGroup) %>% 
  summarise(median = median(as.numeric(Sleep_regular_amount), na.rm =TRUE))
  
# Question 16
health_data_supplement %>%
  group_by(AgeGroup) %>% select(AgeGroup, Sleep_yesterday_amount) %>%
  summarise(median = median(as.numeric(Sleep_yesterday_amount), na.rm =TRUE))

# Question 17
health_data_supplement %>%
  group_by(AgeGroup, Exercise_regular_types) %>% 
  select(AgeGroup, Exercise_regular_types) %>%  count() %>%  print(n=80)
  
# Question 18
health_data_supplement %>%
  group_by(AgeGroup) %>%
  summarise(median = median(as.numeric(Exercise_regular_frequency), na.rm =TRUE))
  
# Question 19
health_data_supplement %>%
  group_by(AgeGroup) %>% select(AgeGroup, Sedentary_regular_time) %>% 
  mutate(Sedentary_regular_time = as.numeric(sub(" .*", "", Sedentary_regular_time))) %>%
  summarise(mean = mean(Sedentary_regular_time, na.rm = TRUE), sd = sd(Sedentary_regular_time, na.rm = TRUE))
...
semmst_data_enc %>% 
  ...
  ggplot(aes(x=response, y=percentage)) + 
    ...
    geom_point(aes(fill = AgeGroup),position=position_jitterdodge()) +
    custom_theme + 
...
semmst_data_rec %>%
  ...
  ggplot(aes(x = factor(stimtype, levels=c("TARG", "CLOSE", "DISTANT", "FOIL")), y = percentage_id, fill=factor(response, levels=c("old", "new", "None")))) +
    ...
    #geom_errorbar(aes(ymin=percentage-sd, ymax=percentage+sd), width=.3,
                 #position=position_dodge(.9), size=0.86) +
    geom_point(aes(x = factor(stimtype, levels=c("TARG", "CLOSE", "DISTANT", "FOIL")), y = percentage_id, fill=factor(response, levels=c("old", "new", "None"))), position = position_jitterdodge(dodge.width=1)) +
    labs(x = "Type of stimulus", y = "Percentages of responses", title = "Percentage of responses given for different\n stimulus conditions in the Recognition phase of the SMST task", fill = "Type of response") +
    scale_y_continuous(breaks = seq(0,1, by =0.2), labels=percent) +
    custom_theme +
    facet_wrap(~AgeGroup, labeller = labeller(AgeGroup=c("OA" = "Old Adults", "YA" = "Young Adults"))) +
    scale_fill_brewer(palette = "Dark2") +
    scale_x_discrete(labels=c("TARG" = "TARGET", 
                              "CLOSE" = "CLOSE\nLURE",
                              "DISTANT" = "DISTANT\nLURE",
                              "FOIL" = "FOIL"))
...
semmst_data_final_clean %>%
  select(ID, AgeGroup, Sex, Age, Beck, Vocabulary, Digit_symbol, Non_word_span, Non_word_sum, Moca, MST_rec_score, MST_ldi, exercise_regular, alcohol_regular, caffeine_regular) %>% #PALT_best_performance_trial, PALT_best_performance_score,
  ...
  ggplot(aes(x = factor(AgeGroup, levels= c("OA", "YA"), labels=c("Old Adults", "Young Adults")), y = as.numeric(result))) +
    ...
    facet_wrap(~questionnaire, ncol = 5, scales = "free", labeller = labeller(questionnaire = c("Non_word_sum"="Non-word repetition raw score",
...
semmst_data_final_clean %>%
  ...
  ggplot(aes(x = factor(stimtype_REC, levels=c("TARG", "CLOSE", "DISTANT", "FOIL")), y = percentage_id, fill=factor(response_REC, levels=c("old", "new", "None")))) +
    ...
    labs(x = "Condition", y = "Percentage of responses", title = "Percentage of responses for conditions in the Recognition phase", fill = "Type of response") +
    scale_y_continuous(breaks = seq(0,1, by =0.2), labels=percent) +
    custom_theme +
    facet_wrap(~AgeGroup, labeller = labeller(AgeGroup=c("OA" = "Old Adults", "YA" = "Young Adults"))) +
    scale_fill_brewer(palette = "Dark2")
...
smst_stim_list_complex %>% 
  ...
  ggplot(aes(x = factor(stimtype_REC, levels= c("TARG", "CLOSE", "DISTANT", "FOIL"), labels=c("Targets","Close\nLures", "Distant\nLures", "Foils")), y = as.numeric(result))) +
    ...
    geom_point(position = position_jitter()) +
    geom_boxplot(aes(fill = stimtype_REC), width=0.2) +
    facet_wrap(~covariate, scales = "free", ncol = 3, labeller = labeller(covariate = c("noun_freq"="Noun frequency",
                                                                              "noun_char" = "Noun characters",
                                                                              "noun_syll" = "Noun syllables",
                                                                              "adj_freq" = "Adjective frequency",
                                                                              "adj_char" = "Adjective characters",
                                                                              "adj_syll" = "Adjective syllables",
                                                                              "ya_ert_mean" = "Meaningfulness (30 YA)",
                                                                              "ya_konk_mean" = "Concreteness (30 YA)",
                                                                              "ya_aro_mean" = "Arousal (30 YA)",
                                                                              "oa_ert_mean" = "Meaningfulness (34 OA)",
                                                                              "oa_konk_mean" = "Concreteness (34 OA)",
                                                                              "oa_aro_mean" = "Arousal (34 OA)"))) +
    custom_theme +
    #geom_text(aes(label=result, fill = 'black'), hjust = -0.5) +
    labs(title = "Item-level covariates", x = "Stimulus Conditions", y = "Results of different item-level covariates") +
    #scale_fill_brewer(palette="Dark2") + 
    scale_fill_manual(values = c("#306EFF", "#FFEF00", "#E41A1C", "#4DAF4A")) +
    theme(legend.position = "none") 
...
smst_stim_list %>%
  ...
  ggplot(aes(x = stim_type, y = cosine_metric)) +
    geom_violin(aes(fill = stim_type)) +
    geom_point(position = position_jitter(width=0.15), size = 3,  shape=1, stroke=2) +
    #theme_bw() +
    custom_theme +
    labs(title = "Cosine Similarity of\nLures to Targets", x = "Type of Lure", y = "Cosine Similarity") +
    scale_fill_manual(values=c("#306EFF", "#FFD700")) +
    theme(legend.position = "none") +
    coord_cartesian(ylim = c(0.35, 1))
...
plot_model(model_complex_participants_cognitive_health, 
           type = "pred",
           terms = c("MST_ldi_scale", "AgeGroup"), #, "stimtype_REC"),
           colors = c("seagreen3", "deepskyblue2", "orchid"),
           title = "Raw probabilities",
           axis.title = c("MST LDI", "p (judged item is OLD)"),
           line.size = 2.0,
           show.data = TRUE,
           jitter = 0.07
           )
...
plot_model(model_complex_participants_cognitive_health, type = "est", transform = "exp", terms = c("(Intercept)",
           ...
           axis.labels = c("Close Lure * Old Adults * YA concreteness score",
                           "Distant Lure * Young Adults * YA concreteness score", 
                           "Close Lure * Young Adults * YA concreteness score",
                           "Target * Noun frequency", 
                           "Foil * Noun frequency", 
                           "Young adults * LDI in MST",
                           "Old adults * Non-word repetition", 
                           "Young adults * Processing speed", 
                           "Age Group * Distant Lure", 
                           "Age Group * Close Lure", 
                           "Age Group * Target", 
                           "Old adult", 
                           "Distant Lure", 
                           "Close Lure", 
           ...
           show.intercept = TRUE) + custom_theme + theme(axis.text.y = element_text(vjust = 0.5, hjust=1, size = 15, face="bold"))
...
plot_model(model_simple, type = "est", transform = "exp", terms = c("(Intercept)",
           ...
           colors = c("seagreen3", "deepskyblue2"), 
           show.values = TRUE,
           value.offset = .4,
           value.size = 6,
           dot.size = 6,
           ...
           width = 0.5,
           wrap.labels = 70,
           axis.labels = c("Age Group * Distant Lure", "Age Group * Close Lure", "Age Group * Target", "Old Adults", "Distant Lure", "Close Lure", "Target", "Intercept (Foil)"),
           axis.title = c("Exponentiated estimates", "x"),
           title = "Simple model",
           order.terms = c(4,6,3,5,1,2),
           group.terms = c(1,1,2,3,2,3),
...
### Figure 1 C - cosine similarities
...
```{r}
semmst_data_rec_se = semmst_data_rec %>%
  filter(response != "None") %>%
  select(AgeGroup, ID, stimtype, response) %>%
  group_by(AgeGroup, ID, stimtype, response) %>% filter(response != "4") %>% 
  count() %>%
  ...
  group_by(AgeGroup, ID, stimtype) %>%
  mutate(percentage_id = n/sum(n)) %>%
  group_by(AgeGroup, stimtype, response) %>%
  summarise(se = sd(percentage_id, na.rm=FALSE) / sqrt(length(percentage_id)),
            percentage_id = mean(percentage_id, na.rm=FALSE))
...
semmst_data_rec %>%
  ...
  ggplot(aes(x = factor(stimtype, levels=c("TARG", "CLOSE", "DISTANT", "FOIL")), y = percentage_id, fill=factor(response, levels=c("old", "new", "None")))) +
    geom_bar(stat = "summary", position = position_dodge(width=1)) +
    geom_errorbar(aes(ymin=percentage_id-se, ymax=percentage_id+se, group=factor(response, levels=c("old", "new"))), width=.2,
                 position=position_dodge(1), size=1.5, data = semmst_data_rec_se) +
    #geom_point(aes(x = factor(stimtype, levels=c("TARG", "CLOSE", "DISTANT", "FOIL")), y = percentage_id, fill=factor(response, levels=c("old", "new", "None"))), position = position_jitterdodge(dodge.width=1, jitter.width = 0.2), size = 3) +
    labs(x = "Stimulus condition", y = "Percentage of responses", title = "Percentage of responses for different stimulus\n conditions in the recognition phase of the sMST", fill = "Type of\nresponse") +
    scale_y_continuous(breaks = seq(0,1, by =0.2), labels=percent) +
    custom_theme +
    facet_wrap(~AgeGroup, labeller = labeller(AgeGroup=c("OA" = "Old Adults", "YA" = "Young Adults"))) +
    scale_fill_brewer(palette = "Dark2") +
...
# Boxplot
dprimes_table %>% 
  ggplot(aes(x = factor(type, labels = c("TARGET \nvs.\nCLOSE LURE", "TARGET \nvs.\nDISTANT LURE", "TARGET \nvs.\nFOIL")), y = dprime)) +
    geom_boxplot(aes(colour = factor(type, labels = c("TARGET \nvs.\nCLOSE LURE", "TARGET \nvs.\nDISTANT LURE", "TARGET \nvs.\nFOIL"))), lwd=1.3) +
    geom_point(aes(x = factor(type, labels = c("TARGET \nvs.\nCLOSE LURE", "TARGET \nvs.\nDISTANT LURE", "TARGET \nvs.\nFOIL")), y = dprime), position = position_jitter(width=0.1), size = 3) +
    #scale_colour_brewer(palette="Dark2") +
    scale_colour_manual(values = c("#306EFF", "#FFEF00",  "#E41A1C")) +
    labs(x = "Type of d'", y = "Discriminability (d')", title = "Discriminability (d') of stimulus conditions in sMST by age group", colour = "Type of d'") +
    custom_theme +
    facet_wrap(~AgeGroup, labeller = labeller(AgeGroup=c("OA" = "Old Adults", "YA" = "Young Adults"))) +
    theme(legend.position = "none")
...
plot_model(model_simple, type = "est", transform = "exp", terms = c("(Intercept)",
           ...
           width = 0.5,
           wrap.labels = 70,
           axis.labels = c("Old Adults * Distant Lure", "Old Adults * Close Lure", "Old Adults * Target", "Old Adults", "Distant Lure", "Close Lure", "Target", "Intercept (Foil, Young Adults)"),
           axis.title = c("", "x"),
           title = "Exponentiated estimates from simple GLMEM",
           order.terms = c(4,6,3,5,1,2),
           group.terms = c(1,1,2,3,2,3),
           show.intercept = TRUE) + custom_theme + theme(axis.text.y = element_text(vjust = 0.5, hjust=1, size = 15, face="bold"))
...
plot_model(model_complex_participants_cognitive_health, type = "est", transform = "exp", terms = c("(Intercept)",
                                                                                ...
                                                                                "stimtype_RECCLOSE:AgeGroupOA:ya_konk_mean_scale",
                                                                                "stimtype_RECCLOSE:AgeGroupYA:ya_konk_mean_scale",
                                                                                "stimtype_RECCLOSE:AgeGroupYA:oa_konk_mean_scale",
                                                                                "stimtype_RECDISTANT:AgeGroupYA:ya_konk_mean_scale"), 
           colors = c("seagreen3", "deepskyblue2", "orchid"), 
           show.values = TRUE,
           value.offset = .4,
           ...
           axis.labels = c("Close Lure * Old Adults * YA concreteness score",
                           "Distant Lure * Young Adults * YA concreteness score",
                           "Close Lure * Young Adults * YA concreteness score",
                           "Close Lure * Young Adults * OA concreteness score",
                           "Target * Noun frequency", 
                           "Foil * Noun frequency", 
                           "Old adults * Non-word repetition", 
                           "Young adults * Processing speed", 
                           "Old adults * Distant Lure", 
                           "Old adults * Close Lure", 
                           "Old adults * Target", 
                           "Old adults", 
                           "Distant Lure", 
                           "Close Lure", 
                           "Target", 
                           "Intercept (Foil, Young Adults)"),
           title = "Exponentiated estimates from complex GLMEM",
           order.terms = c(4,6,3,5,1,2),
           group.terms = c(1,1,2,3,2,3),
           show.intercept = TRUE) + custom_theme + theme(axis.text.y = element_text(vjust = 0.5, hjust=1, size = 15, face="bold"))
...
semmst_data_final_clean_GLMEM %>% 
  ...
  ggplot() +
    ...
    geom_smooth(method = "glm", 
                ...
                linewidth=2) +
    coord_cartesian(ylim = c(0, 1), xlim = c(0.4,1)) +
    scale_color_discrete(name = "Age\ngroup", labels = c("Young\nadults", "Old\nadults")) +
    custom_theme +
    labs(title = "Continous GLMEM", y = "Percentage of 'old' responses", x = "Cosine similarity") +
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), legend.text.align = 0) 


# Plot to compare with Stark et al. (2013, Neuropsychologica)
semmst_data_final_clean_GLMEM %>% 
  mutate(response_REC = ifelse(response_REC == "old", 0, 1)) %>%
  #filter(stimtype_REC != "FOIL") %>% 
  ggplot() +
    aes(y = response_REC, x = cosine_metric, group = AgeGroup, color = AgeGroup) +
    #geom_point() +
    geom_smooth(method = "glm", 
                method.args = list(family = "binomial"),
                linewidth=2) +
    coord_cartesian(ylim = c(0, 1), xlim = c(1,0)) +
    scale_color_discrete(name = "Age\ngroup", labels = c("Young\nadults", "Old\nadults")) +
    custom_theme +
    labs(title = "Continous GLMEM", y = "1 - p|'old'", x = "Cosine similarity") +
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), legend.text.align = 0) 
...
# Separated by AgeGroup - OA n.s., YA sig.
semmst_data_final %>% 
  select(ID, AgeGroup, MST_ldi, lure_dprime) %>% 
  distinct() %>% 
  ggplot(mapping = aes(x = MST_ldi, y = lure_dprime, fill = AgeGroup, colour = AgeGroup)) +
    geom_point(shape = 21, color = 'white', size = 6) +
    scale_fill_discrete(name = "Age Groups", labels = c("Old Adults", "Young Adults")) +
    sm_statCorr(fit.params = list(color = 'black',
                                   linetype = 'dashed'), text_size = 10) +
    custom_theme +
    theme(legend.position = "none") +
    labs(x = "MST LDI", y = "sMST - Lure d'", title = "Correlation of lure discrimination performance in the MST and sMST") +
    guides(colour = FALSE)

# Compare correlations
smst_ldi_corr_YA = semmst_data_final %>% 
  select(ID, AgeGroup, MST_ldi, lure_dprime) %>% 
  distinct() %>% 
  filter(AgeGroup=="YA")

smst_ldi_corr_OA = semmst_data_final %>% 
  select(ID, AgeGroup, MST_ldi, lure_dprime) %>% 
  distinct() %>% 
  filter(AgeGroup=="OA")


cocor(~ MST_ldi + lure_dprime | MST_ldi + lure_dprime,
      data = list(smst_ldi_corr_YA, smst_ldi_corr_OA))
## No sig. difference in correlations: z = 1.28, p = 0.2

# Going ahead with whole sample correlation:
semmst_data_final %>% 
  select(ID, AgeGroup, MST_ldi, lure_dprime) %>% 
  distinct() %>% 
  ggplot(mapping = aes(x = MST_ldi, y = lure_dprime)) +
    geom_point(aes(colour=AgeGroup), size = 6) +
    scale_colour_discrete(name = "Age Groups", labels = c("Old Adults", "Young Adults")) +
    sm_statCorr(fit.params = list(color = 'black',
                                   linetype = 'dashed'), text_size = 10) +
    custom_theme +
    theme(legend.position = "bottom") +
    labs(x = "Discrimination index (LDI) in the perceptual MST", y = "Discrimination index (lure d') in the semantic MST", title = "Correlation of lure discrimination performance\nin the MST and sMST") 
    #guides(colour=guide_legend(theme=theme(
...
# Grand
semmst_data_enc_se = semmst_data_enc %>% 
  select(ID, AgeGroup, stimtype, response) %>% 
  group_by(ID, AgeGroup, stimtype, response) %>%
  filter(response != "None") %>% 
  count() %>%
  group_by(ID, AgeGroup, stimtype) %>%
  mutate(percentage = n/sum(n)) %>% 
  group_by(AgeGroup, stimtype ,response) %>%
  summarise(se = sd(percentage, na.rm=FALSE) / sqrt(length(percentage)),
            percentage = mean(percentage, na.rm=FALSE))
...
semmst_data_enc %>% 
  ...
  group_by(ID, AgeGroup, stimtype) %>%
  mutate(percentage = n/sum(n)) %>% 
  ggplot(aes(x=stimtype, y=percentage)) + 
    geom_bar(aes(fill = response),position=position_dodge(width=1), stat="summary") +
    #geom_point(aes(x=stimtype, y=percentage, fill = response),position=position_jitterdodge(dodge.width=1, jitter.width=0.1), size = 3) +
    geom_errorbar(aes(ymin = percentage-se, ymax = percentage+se, group = response), position=position_dodge(1), width =.2, size = 1.5, data=semmst_data_enc_se) +
    custom_theme + 
    labs(x = "Encoding stimulus type", y = "Percentage of responses", title = "Good or not-good responses in SMST encoding phase ", fill = "Response\nmade") +
    scale_y_continuous(breaks = seq(0,1, by =0.2), limits = c(0,1), labels=percent) +
    scale_fill_brewer(palette="Dark2", labels = c("good", "no-good")) +
    facet_wrap(~AgeGroup, labeller = labeller(AgeGroup=c("OA" = "Old Adults", "YA" = "Young Adults"))) +
    scale_x_discrete(labels=c("TARG" = "TARGET",
                              "FOIL" = "FOIL"))
...
semmst_data_enc %>% 
  ...
  ggplot(aes(x = stimtype, y = reaction_time)) +
    custom_theme +
    geom_violin(aes(fill = response), position = position_dodge(width=1)) +
    geom_boxplot(aes(groupby = response), position = position_dodge(width = 1), width = 0.3, lwd = 2) +
    labs(x = "Encoding stimulus type", 
         y = "Reaction time", 
         title = "Reaction time in SMST encoding phase\nby age group, stimulus type and response", 
         fill = "Response\nmade") +
    facet_wrap(~AgeGroup, labeller = labeller(AgeGroup=c("OA" = "Old Adults", "YA" = "Young Adults"))) +
    scale_fill_brewer(palette = "Dark2", labels = c("good", "no-good")) +
    scale_x_discrete(labels=c("TARG" = "TARGET",
...
```{r}
semmst_data_final_clean %>%
  select(ID, AgeGroup, Sex, Age, Beck, Vocabulary, Digit_symbol, Non_word_span, Non_word_sum, Moca, MST_rec_score, MST_ldi, exercise_regular, alcohol_regular, caffeine_regular) %>% 
  gather(questionnaire, result, -ID, -AgeGroup, -Sex, -Age) %>% 
  filter(questionnaire != 'Moca') %>%
  unique() %>% drop_na() %>% 
  ggplot(aes(x = factor(AgeGroup, levels= c("OA", "YA"), labels=c("Old Adults", "Young Adults")), y = as.numeric(result))) +
...
# Reaction times in REC based on response for conditions - faceted by age groups
semmst_data_rec %>% 
  drop_na(reaction_time) %>% 
  filter(response != "4") %>%
  filter(reaction_time < 3) %>% 
  ...
  ggplot(aes(x = stimtype, y = reaction_time)) +
    custom_theme +
    geom_violin(aes(fill = AgeGroup), position = position_dodge(width=1)) +
    geom_boxplot(aes(groupby = AgeGroup), position = position_dodge(width = 1), width = 0.3, lwd = 2) +
    labs(x = "Recognition stimulus type", 
         y = "Reaction time", 
         title = "Reaction time in SMST recognition phase\nby age group and response", 
         fill = "Age Group") +
    #facet_wrap(~AgeGroup, labeller = labeller(AgeGroup=c("OA" = "Old Adults", "YA" = "Young Adults"))) +
    scale_fill_brewer(palette = "Paired", labels = c("Old\nadults", "Young\nadults")) +
    scale_x_discrete(labels=c("TARG" = "TARGET",
                              "CLOSE" = "CLOSE\nLURE",
                              "DISTANT" = "DISTANT\nLURE",
                              "FOIL" = "FOIL"))
