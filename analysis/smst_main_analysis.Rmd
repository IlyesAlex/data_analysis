...
# Lure discrimination - TARG vs LURE  
...
dprimes_table_wide <- data.frame(semmst_data_rec_wide_dprime$ID, semmst_data_rec_wide_dprime$AgeGroup, rec_dprime$dprime, close_dprime$dprime, dist_dprime$dprime, lure_dprime$dprime) %>% 
  as_tibble() %>%
  magrittr::set_colnames(c("ID", "AgeGroup", "rec_dprime", "close_dprime", "distant_dprime", "lure_dprime"))
...
### FINAL MERGE
semmst_data_final = Reduce(function(x, y) merge(x, y, all=TRUE), list(semmst_data_wider_cosine, mst_sum, nback_sum, palt_sum, heatlh_sum, dprimes_table_wide))
...
# Fixed correlations with only one datapoint per participant
semmst_data_final_clean_youngcorr = semmst_data_final_clean %>% 
  filter(AgeGroup == "YA") %>% 
  select(ID, AgeGroup, MST_ldi, lure_dprime) %>% 
  distinct()
semmst_data_final_clean_oldcorr = semmst_data_final_clean %>% 
  filter(AgeGroup == "OA") %>% 
  select(ID, AgeGroup, MST_ldi, lure_dprime) %>% 
  distinct()

cor.test(semmst_data_final_clean_youngcorr$MST_ldi, semmst_data_final_clean_youngcorr$lure_dprime) # YOUNG
cor.test(semmst_data_final_clean_oldcorr$MST_ldi, semmst_data_final_clean_oldcorr$lure_dprime) # OLD
...
semmst_data_final_clean %>% 
  select(ID, AgeGroup, MST_rec_score, rec_dprime) %>% 
  distinct() %>% 
...
semmst_data_final_clean %>% 
  select(ID, AgeGroup, MST_ldi, close_dprime) %>% 
  distinct() %>% 
  #filter(MST_rec_score > 0.4) %>%
...
semmst_data_final_clean %>% 
  select(ID, AgeGroup, MST_ldi, distant_dprime) %>% 
  distinct() %>% 
  #filter(MST_rec_score > 0.4) %>%
...
semmst_data_rec %>%
  ...
  ungroup() %>%
  filter(response == "old") %>% 
  ggplot(aes(x = factor(stimtype, levels=c("TARG", "CLOSE", "DISTANT", "FOIL")), y = percentage_id, fill = factor(stimtype, levels=c("TARG", "CLOSE", "DISTANT", "FOIL")))) +
    geom_bar(stat = "summary", position = position_dodge(width=1)) +
...
<<<<<<< Updated upstream
    labs(x = "Type of stimulus", y = "Percentage of 'old' response", title = "Percentage of 'old' response given for different\n stimulus conditions in the Recognition phase of the SMST task") +
=======
...
>>>>>>> Stashed changes
    ...
    theme(legend.position = "none") +
    facet_wrap(~AgeGroup, labeller = labeller(AgeGroup=c("OA" = "Old Adults", "YA" = "Young Adults"))) +
    scale_fill_manual(values = c("#4DAF4A", "#306EFF", "#FFEF00", "#E41A1C")) +
    scale_x_discrete(labels=c("TARG" = "TARGET", 
                              "CLOSE" = "CLOSE\nLURE",
                              "DISTANT" = "DISTANT\nLURE",
                              "FOIL" = "FOIL"))
...
>>>>>>> Stashed changes
    scale_fill_discrete(name = "Age Groups", labels = c("Old Adults", "Young Adults")) +
    sm_statCorr(fit.params = list(color = 'black',
                                   linetype = 'dashed'), text_size = 15) +
    custom_theme +
    theme(legend.position = "none") +
    labs(x = "MST LDI", y = "sMST - Lure d'", title = "Correlation of d' in the sMST\nand the LDI scores in the MST") +
    guides(colour = FALSE)
